...
async def get_projects(
    ...
    search: Annotated[
        Optional[str],
        Query(title="Partial Name Search", description="Case-insensitive search on project name."),
    ] = None,
    ordering: Annotated[
        Optional[str],
        Query(title="Ordering", description="Sort field; prefix with '-' for descending order."),
    ] = None,
...
@router.post(
    ...
    responses={**PROJECT_CREATE_RESPONSES, **AUTHENTICATION_RESPONSES},
...
async def create_project(
    ...
    return ProjectInDB.model_construct(**project.__dict__)
...
@router.get(
    ...
    responses={**NOT_FOUND_RESPONSE, **AUTHENTICATION_RESPONSES},
)
...
async def get_project_files(
    ...
    return [FileInDB.model_construct(**file.__dict__) for file in files]
...
async def get_step(
    ...
    return StepWithRelations.model_construct(**step.__dict__)


@router.post(
    "/{project_id}/steps/{step_num}/attempts",
    tags=[STEPS_TAG],
    responses={**AUTHENTICATION_RESPONSES, **NOT_FOUND_RESPONSE, **STEP_START_ATTEMPT_RESPONSES},
    response_model=StepWithRelations,
)
async def start_step_attempt(
    ...
    step = await service.start_step(project_id=project_id, step_num=step_num, user_team_id=user_and_team_id[1])
    return StepWithRelations.model_construct(**step.__dict__)
...
@router.patch(
    "/{project_id}/steps/{step_num}/attempts",
    tags=[STEPS_TAG],
    responses={**MODIFY_STEP_ATTEMPT_RESPONSES, **AUTHENTICATION_RESPONSES, **NOT_FOUND_RESPONSE},
    response_model=StepWithRelations,
)
async def modify_step_attempt_state(
    ...
    match data.action:
        case "set-timer":
            step = await service.set_step_timer(project_id=project_id, step_num=step_num, timer=data.timer)
        case "accept":
            step = await service.accept_step(project_id=project_id, step_num=step_num, score=data.score)
        case "reject":
            step = await service.reject_step(project_id=project_id, step_num=step_num, timer=data.timer)
        case _:
...
async def get_comments(
    ...
    comments = await service.get_comments(project_id=project_id, step_num=step_num, user=current_user)
    return [StepCommentInDB.model_construct(**c.__dict__) for c in comments]
...
async def create_comment(
    ...
    comment = await service.create_comment(project_id=project_id, step_num=step_num, text=text, files=files, user=current_user)
    return StepCommentInDB.model_construct(**comment.__dict__)
...
async def delete_comment(
    ...
    await service.delete_comment(project_id=project_id, step_num=step_num, comment_id=comment_id, user=current_user)


@router.get(
    "/{project_id}/files/{document_name}",
    tags=[PROJECT_TAG],
    responses={**AUTHENTICATION_RESPONSES, **NOT_FOUND_RESPONSE},
